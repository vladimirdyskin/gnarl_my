...
static int perform_wakeup_burst(const uint8_t *id_packet) {
    ...
    wakeup_pkt[0] = id_packet[0];
    wakeup_pkt[1] = id_packet[1];
    wakeup_pkt[2] = id_packet[2];
    wakeup_pkt[3] = id_packet[3];
    wakeup_pkt[4] = 0x5D;
    wakeup_pkt[5] = 0x00;
    ...
    int n = 0;
    for (int i = 0; i < 150; i++) {
        // if (cancel_current_command) break; // cancel_current_command not visible here? It is static volatile in gnarl.c
        // It is defined at top of file.
        
        transmit(tx_data, tx_len);
        usleep(500);
        n = receive(rx_buf.packet, sizeof(rx_buf.packet), 16);
        
        if (n > 0) {
            ESP_LOGI(TAG, "Wakeup ACK received at burst %d", i);
            last_pump_comm_time = esp_timer_get_time();
            break;
        }
    }
...
static void send_and_listen(const uint8_t *buf, int len) {
	...
	bool is_wakeup = (len > 4 && p->packet[4] == 0x5D);
	int n = 0;
	int rssi = 0;
	...
	if (is_wakeup) {
		...
		// Auto-wakeup check
		if (esp_timer_get_time() - last_pump_comm_time > 60 * SECONDS) {
			ESP_LOGI(TAG, "Auto-wakeup triggered (>60s silence)");
