...
bool pump_wakeup(void) {
	...
	// Background use: keep wakeup bounded so it doesn't monopolize the radio.
	// The caller can retry periodically.
	const int WAKEUP_FINAL_TIMEOUT_MS = 1500;
	encode_short_packet(CMD_WAKEUP);
	int n;
	perform(CMD_WAKEUP, short_buf, sizeof(short_buf), 100, 10, CMD_ACK, &n);
	uint8_t *data = perform(CMD_WAKEUP, short_buf, sizeof(short_buf), 1, WAKEUP_FINAL_TIMEOUT_MS, CMD_ACK, &n);
	return data != 0;
}
